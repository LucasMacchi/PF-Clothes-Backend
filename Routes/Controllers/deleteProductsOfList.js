...
const deleteProductsOfList = async(productID, userID, order) => {
    if(order === "fav"){
        ...
        if(!data.dataValues.favorites.length) throw Error("Este perfil no tiene ningun producto en favoritos")
        data.dataValues.favorites = data.dataValues.favorites.filter((prod) => prod !== productID)
        ...
        const data = await user
        if(!data) throw Error("Ese perfil no existe")
        if(!data.dataValues.shoppingCart.length) throw Error("Este perfil no tiene ningun producto en el carrito")
        data.dataValues.shoppingCart = data.dataValues.shoppingCart.filter((prod) => prod !== productID)
